...
Catalogue search results use a tight artwork grid. Pause the pointer over a card for a 200% preview; click or tap for the 300% artwork viewer. Zoom with the wheel, pinch or buttons, and pan within the image. A completed outside click/tap, Escape or browser Back closes the viewer. **Card details** opens the dedicated printing and ownership page; Back restores the search. Owned cards and autocomplete still open their card page directly.

Drag a card, hold it briefly on touch, or use its **⋯ Card actions** button (keyboard: Shift+F10). A small offset copy follows the gesture while the original stays in place. The wheel prioritizes recent/relevant tags, keeps targets fixed during a drag and provides searchable **More tags**. Escape or releasing outside a target cancels. A location action assigns one copy; from a location view it moves one assigned copy. Classification tags never consume copies, and owned totals stay unchanged. **Edit tags** opens the existing quantity editor.
