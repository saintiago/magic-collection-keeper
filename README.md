...
Collection, deck/tag, catalogue and Home tiles show only artwork at rest. Mouse hover reveals translucent printing, quantity and tag information on the card; a continuous 200ms hover opens a 200% preview. Both the tile and visible enlarged artwork tilt continuously with pointer position. Click or tap opens the 300% inspector without changing the current route, with details on the left and controls on the right (stacked artwork above the two panels on phones). Zoom with the wheel, pinch or buttons, and pan within the image. A completed outside click/tap, Escape or browser Back closes the viewer. **Card details** opens the dedicated printing and ownership page; Back restores the search. Autocomplete and recent-search selections still open the dedicated card page directly. Owned quantities can be saved in the inspector; soft allocation reminders never block edits.

Drag a card directly, or hold it briefly on touch (keyboard pickup: Shift+F10). A full-size translucent copy follows the pointer while the original stays in place. There is no Card actions menu button. The larger radial wheel surrounds the pickup context, adjusts once for viewport edges and keeps wrapped labels readable. The wheel prioritizes recent/relevant tags, keeps targets fixed during a drag and provides searchable **More tags**. Escape or releasing outside a target cancels. A location action assigns one copy; from a location view it moves one assigned copy. Classification tags never consume copies, and owned totals stay unchanged. **Edit tags** opens the existing quantity editor.
