...
Collection, deck/tag, catalogue and Home tiles show only artwork at rest. Hover reveals collection quantities and plain tag names; a continuous 300ms hover lifts one opaque card toward twice its rendered tile size, capped to leave 16px at viewport edges. The visible artwork tilts with the pointer. Click or tap lifts the full portrait in place to up to three times the tile size, preserving the source slot and surrounding grid under subtle dimming. It stays centered on its source, shifting only as needed to fit inside the actual visible viewport with at least 24px side and 32px vertical dismissal margins plus safe-area insets. Information stays beside the card on the left and individual glass actions beside it on the right, overlapping the image when needed on phones. Printed names, set/collector details and rules remain on the card and its dedicated details page. Zoom with wheel, pinch or buttons, and pan within the image. The displayed percentage is the actual source-relative scale; select it to reset to the comfortable opening fit. Outside gutters remain available after deliberate enlargement. A completed outside click/tap, Escape or browser Back closes the viewer without activating a card behind it. **Card details** opens the dedicated printing and ownership page; Back restores the search. Autocomplete and recent-search selections still open the dedicated page directly. Owned quantities can be saved in the inspector; soft allocation reminders never block edits.

Drag a card directly, or hold it briefly on touch (keyboard pickup: Shift+F10). A full-size translucent copy follows the pointer. One opaque pickup image moves into the wheel center while its source tile is hidden, shrinking only when needed to keep labels clear; cancellation restores the tile. There is no Card actions menu button. Translucent glass wedges surround the pickup context, with an open center and a glow that follows the pointer. The picked card stays prominent while neighbouring cards dim. The wheel adjusts once for viewport edges, overlaps more on small screens, and keeps its hit sectors fixed. Recent/relevant tags come first. **More tags** contains the complete searchable list and other card actions; long labels that cannot fit a sector stay readable there. Escape or releasing outside a target cancels. A location action assigns one copy; from a location view it moves one assigned copy. Classification tags never consume copies, and owned totals stay unchanged. **Edit tags** opens the existing quantity editor.
