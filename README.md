...
Collection, deck/tag, catalogue and Home tiles show only artwork at rest. Mouse hover reveals translucent printing, quantity and tag information on the card; a continuous 200ms hover lifts one opaque card to exactly twice its rendered tile size. Both the tile and visible enlarged artwork tilt continuously with pointer position. Click or tap springs into a 300% inspector—exactly three times that rendered tile size—without changing the current route, with details on the left and controls on the right (overlaid on the artwork on phones). The full viewport remains available to the artwork; exact scaling may crop its edges. Zoom with the wheel, pinch or buttons, and pan within the image. A larger full-card image loads without changing the displayed size. A completed outside click/tap, Escape or browser Back closes the viewer. **Card details** opens the dedicated printing and ownership page; Back restores the search. Autocomplete and recent-search selections still open the dedicated card page directly. Owned quantities can be saved in the inspector; soft allocation reminders never block edits.

Drag a card directly, or hold it briefly on touch (keyboard pickup: Shift+F10). A full-size translucent copy follows the pointer while the original stays in place. There is no Card actions menu button. Translucent glass wedges surround the pickup context, with an open center and a glow that follows the pointer. The picked card stays prominent while neighbouring cards dim. The wheel adjusts once for viewport edges, overlaps more on small screens, and keeps its hit sectors fixed. Recent/relevant tags come first. **More tags** contains the complete searchable list and other card actions; long labels that cannot fit a sector stay readable there. Escape or releasing outside a target cancels. A location action assigns one copy; from a location view it moves one assigned copy. Classification tags never consume copies, and owned totals stay unchanged. **Edit tags** opens the existing quantity editor.
