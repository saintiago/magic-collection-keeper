...
Recognition is an assisted capture tool, not guaranteed identification. Glare, sleeves, older layouts and small text can require correction. SCAN-10 checks for a stable detailed single-card image, then ignores a recognition matching the last accepted canonical card. An empty tray does not reset that identity. Slide in a different card, or use quantity for consecutive identical copies. Camera movement, glare, backgrounds and multiple cards can produce extra or unclear readings. Keep one card in the guide and review the pending import. Camera reads run one at a time with a one-second minimum retry cadence; repeated/unclear readings back off up to three seconds, while movement can shorten the wait. Every 50 accepted captures saves a pending batch and continues automatically. There is no accepted-card or attempt session cutoff. The wheel holds at most 50 current captures and ten recent saved captures; Review opens earlier batches using Previous/Next. Storage or network failures pause new captures with Retry saving, retaining the same operation identity. These changes are locally implemented, pending deployment verification. Account-saved pending imports survive browser storage clearing; unstaged local captures do not. [SCAN-10 evidence](tests/performance/SCAN10-REPAIR.md) separates local recorded playback from physical-device acceptance.
