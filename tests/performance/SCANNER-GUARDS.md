...
Final scheduling uses `scan-admission`: cheap visual tracking continues every 120 ms independently of the geometry worker. It credits an existing stable interval and admits only a checked frame from the same uninterrupted scene, at most 12000 ms old. A scene change or a raw sampling gap over 500 ms rejects old geometry; the accepted-card anchor survives overlap. A bounded adjacent-frame buffer chooses a sharper snapshot. The selected stability window is 600 ms after comparing 720/600/480 ms on the recorded video, with the same geometry guard. This replaces the earlier serial check plus delay; worker processing does not start a second hold interval.

Scan entry prepares the geometry worker, and camera/photo startup reuses that preparation. Ready requires both geometry and primary recognition preparation. Linux WebKit CI measured approximately 8.5 seconds of geometry initialization; the previous shared 12-second request limit canceled the first check. Geometry requests now have a 35-second outer bound including startup. These are desktop/CI limits, not measured phone latency.
...
### Deployed September 9 verification

Recognition version 8 passed actual authenticated primary/independent identity checks, unauthorized/body-owner/invalid-attempt rejection, decoded-byte and four-megapixel bounds, and two-card/overlap rejection before model proposal. The downloaded source matched the audited archive. Warm sequential control requests took 1.05 s primary and 1.40 s independent; concurrent reads both succeeded at 11.87 s and 2.28 s respectively. These small samples are not latency guarantees.

The first actual POST after the image update failed with HTTP 503 after 29.6 s. CloudWatch recorded a 10 s library initialization timeout followed by a 20 s invocation timeout; the next primary request recovered in 8.1 s. Cold startup can therefore still fail and require retry. No provisioned/reserved compute or periodic warming is enabled. Browser recognition and bounded entry preparation remain available; a failed/unresolved request must never create ownership or a success cue.

The final Linux browser run passed 82 Chromium and 50 WebKit tests, with one explicitly unsupported generated-camera-stream case skipped in WebKit. Its real-photo geometry worker took 7.08 s to initialize and 8.64 s for the first check, completing successfully with preserved preparation. Windows WebKit's earlier 230 ms checks do not describe Linux CI or physical iPhone latency.
