...
Main CI builds and validates the compact index, then publishes its immutable hash path before `/catalog/current.json`. The separate `.github/workflows/catalog.yml` refreshes daily at 06:23 UTC (or manually with workflow_dispatch), runs discovery unit/real-bulk checks and publishes using the existing main-scoped OIDC role and site bucket. Main and catalog publication share a job concurrency group, and older source timestamps cannot replace newer ones. Failed jobs retain the prior catalog. No Lambda/Cognito/CloudFormation/CI permission expansion is needed; owned partitions are never touched. GitHub Actions scheduling can be delayed, so the UI displays the actual source date.

The initial September8 snapshot contains 37,564 English identities, 248,290 extra aliases, about 5.6 MB compressed. It is fetched by Lambda, not by each browser. Daily processing reads about 392 MB of official bulk data; the compact refresh adds one immutable catalog object, with no card images/full oracle database in it. Old compact snapshots are retained for safe publication; review retention deliberately if storage grows. Servers refresh within 24h of a successful scheduled publication (up to roughly 48h from a new upstream name). A warm server retains old data on refresh failure and labels it; a new server returns a retryable error if no valid index is available.

Autocomplete caches only public names in page memory for five minutes, so a previously typed prefix does not add another day of staleness; full search uses the existing shared Scryfall 24h card cache plus a bounded process cache. No API requests are sent upstream for keystrokes. Query results have an honest names-updated date. Keep existing 600 ms spacing and429 cooldown when adjusting this flow. Scryfall’s official [access FAQ](https://scryfall.com/docs/faqs/i-m-having-trouble-accessing-the-scryfall-api-or-i-m-blocked-17) recommends bulk for high-volume simple lookups and proper User-Agent/Accept headers. The rate-limit/bulk documentation HTML returned403 during the September8 recheck, while the official bulk metadata endpoint returned200 with its current all-language JSONL URL; this change retains the stricter existing search limit.
