...
class PaddleText:
    def __init__(self, root):
        ...
        for name, files in manifest.items():
            for filename, digest in files.items():
                if (
                    hashlib.sha256(
                        (root / "ocr-models" / name / filename).read_bytes()
                    ).hexdigest()
                    != digest
                ):
                    raise ValueError("Text model digest mismatch")
        self.model = PaddleOCR(
            text_detection_model_dir=str(root / "ocr-models/PP-OCRv5_mobile_det"),
            text_recognition_model_dir=str(
                root / "ocr-models/latin_PP-OCRv5_mobile_rec"
            ),
        ...
        self.version = {
            ...
            "package": "3.7.0",
            "weights": manifest,
            "calibration": "research-text-v1-not-approved",
        }
    ...
    def read(self, image, corners):
        ...
        flat = cv2.warpPerspective(
            bgr, cv2.getPerspectiveTransform(pts, dst), (800, 1120)
        )
        regions = []
        for a, b in ((0.02, 0.12), (0.92, 1)):
