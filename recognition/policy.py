...
def printing_agrees(printing, footer):
    ...
    return bool(number and re.search(r'(?<!\w)0*'+re.escape(number)+r'(?!\w)',text) and all(re.search(r'(?<!\w)'+re.escape(printing.get(key,'?'))+r'(?!\w)',text) for key in ('set','lang')))

def decide(visual, text, *, allow_confirmed=False):
    evidence={'visual':visual.get('evidence',{}),'text':text.get('evidence',{})}
    base={'contractVersion':1,'candidates':[],'selected':None,'evidence':evidence}
    ...
    if allow_confirmed and visual.get('confirmation_calibrated') and text.get('confirmation_calibrated') and exact:
        return {**base,'status':'confirmed','candidates':candidates,'selected':first,'reason':'printing_corroborated'}
    return {**base,'status':'possible','candidates':candidates,'reason':'review_printing' if title_agrees else 'review_identity'}
