...
class RecognitionService:
    ...
    def recognize(self, image):
        started=perf_counter();timings={};versions={'visual':self.visual.version,'ocr':self.ocr.version}
        try:
            step=perf_counter();visual=self.visual.inspect(image);timings['visualMs']=(perf_counter()-step)*1000
        except Exception:
        ...
        if not visual.get('identity_supported'):
            ...
            try:
                step=perf_counter();text=self.ocr.read(image,visual.get('corners'));timings['ocrMs']=(perf_counter()-step)*1000
            except Exception:
                text={'title':[],'footer':[],'evidence':{'error':'ocr_unavailable'}}
    ...
    def _failure(reason,started,timings,versions):
        return {'contractVersion':1,'status':'unknown','candidates':[],'selected':None,'reason':reason,'evidence':{},'versions':versions,'timings':{**timings,'totalMs':(perf_counter()-started)*1000}}
