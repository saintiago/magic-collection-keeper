...
function scanBatchNavigation(session, busy) {
  if (!session) return "";
  return `<nav class="scan-batch-navigation" aria-label="Scan batches"><span>Batch ${session.index} of ${session.batches} · ${session.pending_batches} pending</span><button type="button" class="secondary" data-draft="${esc(session.previous || "")}" ${busy || !session.previous ? "disabled" : ""}>Previous batch</button><button type="button" class="secondary" data-draft="${esc(session.next || "")}" ${busy || !session.next ? "disabled" : ""}>Next batch</button></nav>`;
}
